...
```text
请执行 [XXX任务]。开始前先完整阅读本仓库的 GUIDE.zh-CN.md，并使用 update-progress-target 为当前会话建立独立计划。所有时间统一使用北京时间 UTC+08:00，并采用带 `+08:00` 的 ISO 8601 格式。按真实依赖拆分语义阶段，不要机械四等分；每阶段必须设置 deadlineAt、结构化质量硬目标、可供下一阶段消费的必需交付物及验收证据、executionPlan 和资源分支。
