  ...
  ctx.tools.register({
    ...
    output: {
      ...
      render(_args, value) {
        ...
        if (value.mustContinue && !value.requiresUserInput) lines.push('必须自主继续，不要等待用户审批；不得因 continuation 或重试轮数停止。')
    ...
    async execute(args) {
      ...
      try {
        ...
        if (next.status === 'overdue' && next.deliverablesReady) {
          ...
          nextAction = '继续当前阶段，按 attempt 调整并重跑；未超时不得提前推进，也不得因 continuation/重试轮数或自动续跑预算停止。'
        }
        return {
          success: true,
