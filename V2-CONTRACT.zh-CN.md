...
阶段目标必须能够区分质量好坏，不能只证明对象存在。`数量 > 0`、`文件数 > 0`、`结果数 > 0` 等目标只能作为 `process` 指标，不能充当质量门；如果一个阶段的全部质量指标都只是 `> 0` 或 `>= 0`，插件会拒绝该阶段。使用 `adaptive` 阈值时，应先在调研或 pilot 阶段测量基线，随后将阈值冻结为具有正式依据的其他类型，再进入生产执行阶段。
